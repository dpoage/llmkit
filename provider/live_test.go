...
// Package provider live conformance probe.
//
...
//
// It builds the client exactly the way production does (a Spec of
// type "openai-compatible" routed through provider.New), so it exercises the
...
func (c *captureRecorder) events() []llmkit.UsageEvent {
	c.mu.Lock()
	defer c.mu.Unlock()
	out := make([]llmkit.UsageEvent, len(c.evs))
	copy(out, c.evs)
	return out
}
...
// os.Getenv so the provider package stays free of config imports.
...
// newLiveClient builds a Client the way production does: a Spec of
...
func TestLive_ToolRoundTrip(t *testing.T) {
	...
	if first.StopReason != llmkit.StopToolUse {
		t.Fatalf("turn 1 StopReason = %q, want %q (model did not request the tool); text=%q",
			first.StopReason, llmkit.StopToolUse, first.Text)
	}
