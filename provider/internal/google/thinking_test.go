...
// encoding/json re-decodes a nil []byte as literal "null"), "null", and "{}"
// are all SKIPPED — no empty part reaches the wire and no error is raised.
// This is deliberately softer than anthropic's guard, which rejects a
// missing Raw outright: anthropic thinking is signed and mandatory on
// replay, while a decoded-to-empty genai Part contributes nothing Gemini
// requires. A malformed Raw is still an error.
