...
| `StructuredOutput` | Dropped silently | `Request.ResponseSchema` is omitted from the wire. When true, OpenAI gets `response_format` and Anthropic gets a synthetic forced-output tool. | Adapter, at request build |
...
| `Documents` | Advisory | Nothing: no adapter reads it. Document blocks pass through like image blocks. | Caller |
...
`provider/capability_gate_test.go` pins the two behavioral classes against every adapter: a profile with `ToolChoice=false` rejects each explicit mode pre-wire, and a profile with `Thinking=false` drops the thinking config from the wire body.
