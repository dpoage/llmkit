...
| `Thinking` | Dropped silently | `Request.Thinking` is omitted from the wire and no thinking blocks are returned. When true, a non-positive `BudgetTokens` is refused pre-wire, and on Anthropic so is `Thinking` combined with `ResponseSchema`: extended thinking admits only `tool_choice` auto or none, while the synthetic structured-output tool forces a named one. | Adapter, at request build |
