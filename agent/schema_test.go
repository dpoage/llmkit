...
// BenchmarkFuncTool_Run measures funcTool.Run's steady-state cost. The schema
// is parsed once into a [parsedSchema] at Func construction, then reused on
// every call — only the per-call args body is parsed.
